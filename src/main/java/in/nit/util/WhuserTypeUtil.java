...
public class WhuserTypeUtil {
	...
	public void generatePie(String path,List<Object[]> data){
		...
		JFreeChart chart=ChartFactory.createPieChart3D("WhuserCode", dataset, true, true, false);
		//3.save as Image using ChartUtils
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/WhuserA.jpg"), chart, 600, 500);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createBarChart("WhuserTypeCode", "Xodes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try {
			ChartUtils.saveChartAsJPEG(new
					File(path+"/resources/images/WhuserB.jpg"), chart, 500, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
